...
新規開発では「コードを書けても公開できない」状態が続くことが障壁になります。
CDNの設定・HTTPS化・キャッシュ制御といったインフラ作業が完了するまで、
検証環境への公開が待ちになるためです。
...
スケールや可用性の心配をアプリ開発者がしなくてよい設計にします。
ECS Fargateはトラフィックに応じてコンテナ数を自動調整し、DynamoDBはレプリケーションとバックアップをマネージドで提供します。
データ構造はスキーマレスのため、「試してみて、違ったら直す」を繰り返しやすいです。

### CI/CDパイプラインによるデプロイの標準化

手作業による事故を防止し、開発状況を常に可視化するため、コード変更をトリガーとした自動ビルド・デプロイ環境を内包しています。
これにより、常に「動作するコード」が環境に反映される状態を維持します。
